...
LIGHT_CONTROL_FILE = "燈控紀錄.xlsx"  # 新增的燈控記錄 Excel
...
def reply_quick_menu(reply_token):
    message = TextSendMessage(
        ...
        quick_reply=QuickReply(items=[
            ...
            QuickReplyButton(action=MessageAction(label="開燈", text="開燈")),
            QuickReplyButton(action=MessageAction(label="關燈", text="關燈"))
        ])
    )
...
def save_light_control_record(user_id, action):
    try:
        if not os.path.exists(LIGHT_CONTROL_FILE):
            wb = Workbook()
            sheet = wb.active
            sheet.title = "燈控紀錄"
            sheet.append(["日期", "時間", "用戶ID", "動作"])
            wb.save(LIGHT_CONTROL_FILE)
        ...
        sheet.append([date, time_str, user_id, action])
        wb.save(LIGHT_CONTROL_FILE)
        print(f"✅ 記錄燈控動作：{user_id} - {action}")
    except Exception as e:
        print(f"❌ 寫入燈控紀錄失敗：{e}")
...
def handle_text_message(event):
    ...
    if text == "選單":
        ...
        line_bot_api.reply_message(
            reply_token,
            TextSendMessage(text="📤 請先輸入您想綁定的名字（必須跟Excel中名字一致），然後傳送一張圖片（請僅傳送一張圖片）。")
        )
        ...
        save_light_control_record(user_id, text)
        line_bot_api.reply_message(reply_token, TextSendMessage(text=f"✅ 已記錄：{text}"))
        # 可選：推播燈狀態給所有用戶
        push_to_all_users(f"💡 燈狀態更新：{text}（由用戶 {user_id} 操作）")
    else:
...
def handle_image_message(event):
    ...
    if user_id in user_states and user_states[user_id].get("step") == "等待圖片":
        ...
        line_bot_api.reply_message(reply_token, TextSendMessage(text="⚠️ 您已完成圖片設定或尚未開始設定，請輸入『我要設定圖片與名字』重新開始。"))

# --------------------------------
# 監控 Excel 新增資料並推播（出入紀錄）
