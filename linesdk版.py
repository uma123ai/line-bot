...
def get_latest_entries_for_all():
    try:
        ...
        for row in range(max_row, 1, -1):
            values = [cell.value for cell in sheet[row][:3]]  # 讀前三欄: 日期(含時間)、名字、狀態
            if len(values) < 2:
                continue
            date_raw, name = values[0], values[1]

            if not date_raw or not name:
                continue  # 空值跳過

            # 日期欄直接格式化成 'YYYY/MM/DD HH:MM'
            try:
                date_fmt = date_raw.strftime("%Y/%m/%d %H:%M")
            except Exception:
                date_fmt = str(date_raw)

            if name not in last_records:
                last_records[name] = date_fmt
        ...
        try:
            ...
            if current_last_row > last_row_num:
                ...
                for row in range(last_row_num + 1, current_last_row + 1):
                    ...
                    # 安全地解包欄位
                    if len(values) == 3:
                        date, time_str, name = values
                        status = "未知"  # 若沒有 status 欄，就補個預設值
                    else:
